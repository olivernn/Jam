...
Jam={version:"0.0.1"};Jam.GridView=function(b,c,f){f=f||{};c=c||{};var i=function(a,h){this.eventHandler=h?$(h):$(document);this.eventNamespace=b+":gridView";this.name=b;this.collection=a;this.page=1;this.settings=c};$.extend(i.prototype,Jam.GridView.instanceMethods,f);return i};
...
a.settings.pageItems);return a.pagesRequired}var a=this;$.extend(this.settings,Jam.GridView.defaults);this.html=$(this.settings.templateSelector).clone();this.holder=$(this.settings.holderSelector);var h=$('<ul class="grid-view-page clearfix"></ul>'),l=$('<li class="grid-view-item"></li>').css({"float":"left",display:"inline"});if(a.collection.length>0){for(var k=1;k<=i();k++)c(k);f();a.eventHandler.bind("pageAnimationStart."+this.eventNamespace,function(){f()});b()}else a.settings.blankStateHtml&&
a.html.find(".grid-page-holder").append(h.clone().append(a.settings.blankStateHtml));a.holder.css({width:a.settings.pageWidth});a.html.find(".grid-page-holder").css({width:"10000px",position:"absolute"}).end().find(".grid-view-port").css({overflow:"hidden",position:"relative",width:a.settings.pageWidth,height:a.settings.pageHeight}).end().find(".grid-view-page").css({padding:"0px","float":"left",width:a.settings.pageWidth});return this.html.addClass(this.name)},insertHtml:function(){this.holder.html(this.html)},
render:function(){this.generateHtml();this.insertHtml()},remove:function(){this.holder.find("."+this.name).remove();this.eventHandler.unbind(this.name+":gridView")},showPage:function(b){var c=this;b=b;if(b<=c.pagesRequired&&b>0){this.page=b;this.eventHandler.trigger("pageAnimationStart."+this.eventNamespace,b);this.html.find(".grid-page-holder").animate({left:-1*(b-1)*parseInt(c.settings.pageWidth)+"px"},this.settings.paginationSpeed,this.settings.paginationEasing,function(){c.eventHandler.trigger("pageAnimationEnd."+
c.eventNamespace,b)});b>=c.pagesRequired-1&&this.eventHandler.trigger("collectionItemsNeeded."+this.eventNamespace)}else throw"cannot show a page that doesn't exist";},updateCollection:function(b){this.collection=b;this.render()}};Jam.GridView.defaults={paginationSpeed:1E3,paginationEasing:"swing",templateSelector:"#templates .grid-container"};
